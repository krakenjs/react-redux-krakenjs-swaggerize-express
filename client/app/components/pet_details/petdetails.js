...
const PetDetails = props => {
    let indicators = props.photoUrls && props.photoUrls.map((url, i) => {
        return (
            <li key={i} data-target="#petdetails" data-slide-to={i} className={i === 0 ? "active": ""}></li>
        )
    });

    let slides = props.photoUrls && props.photoUrls.map((url, i) => {
        return (
            <div key={i} className={i === 0 ? "item active" : "item"}>
                <img src={url} alt={props.name} className="center-block"/>
